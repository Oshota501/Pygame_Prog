...
def normal_font (text:str,position:tuple[float,float]|None=None,font_size=40) -> UI_2d :
    result = UI_2d.text(
            text=text,
...
class StartScene(Scene):
    ...
    def __init__(self,resolution) -> None:
        ...
        self.add_children(
            ...
            self.title ,
            normal_font("Start to Press Space key",(resolution[0]*0.5,resolution[1]*0.5)),
            normal_font("製作者: oshota",(resolution[0]*0.5,resolution[1]*0.65),font_size=24)
        )

    def update(self, delta_time: float):
        self.time += delta_time
        self.title_pos = (self.resolution[0]*0.5,self.resolution[1]*0.3 - abs(math.sin(self.time*2.4)*50))
