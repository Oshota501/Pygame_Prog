...
class Mesh2dMaterial(MaterialLike):
    ...
    def __init__(self):
        ...
        if program is None:
            raise ValueError(
                "Shader Program not found. Ensure Mesh2dShaderContainer is initialized."
            )
    ...
    def add_texture(
        self, texture: TextureLike, location: int, uniform_name: str = "texture0"
    ):
...
class Mesh2d(MeshRender, MeshLike):
    ...
    def get_render_obj(self) -> tuple[moderngl.Context, moderngl.Program] | None:
        if self.shader.context is None or self.shader.program is None:
            return None
        return (self.shader.context, self.shader.program)
    ...
    @staticmethod
    def color_rectangle(
        color: tuple[float, float, float, float], size: Vector2
    ) -> "Mesh2d":
