...
from PyGame3d.Draw.shader_container import ShaderContainaer3dComponent, ShaderContainer
...
class UV3dShaderContainer(
    ShaderContainer, ShaderContainaer3dComponent, metaclass=SingletonABCMeta
):
    def __init__(
        self,
        ...
        for line in file:
            ...
            # --- 面情報の構築 ---
                ...
                for i in range(1, len(parts)):
                    ...
                    # 8要素データを一時リストへ
                    vert_list.append(
                        [xyz[0], xyz[1], xyz[2], uv[0], uv[1], nm[0], nm[1], nm[2]]
                    )
...
class UV3dMeshSub(MeshRender, MeshLike):
    ctx: moderngl.Context
    shader: UV3dShaderContainer
    material: UVMaterial
    vbo: moderngl.Buffer
    ...
    def __init__(self, material: UVMaterial, mesh_data: np.ndarray) -> None:
        ...
        if static.context is None:
            raise ValueError(
                "Please execute Application.init() before creating UV3dMesh"
            )

        self.ctx = static.context
        self.shader = UV3dShaderContainer()
    ...
    @staticmethod
    def load_obj(obj_filename: str) -> list[UV3dMeshSub]:
        objs: list[tuple[str, np.ndarray]] = load_obj(obj_filename)
        result: list[UV3dMeshSub] = []
        for obj in objs:
            result.append(UV3dMeshSub(UVMaterial.img_mono_texture(obj[0]), obj[1]))
        return result

    @staticmethod
    def cutting_boad(texture_name: str) -> UV3dMeshSub:
    ...
    @staticmethod
    def floor_mesh(color=(0.6, 0.6, 0.6), size=(20, 20)) -> UV3dMeshSub:
...
class UV3dMesh(MeshRender, MeshLike):
    sub_mesh: list[UV3dMeshSub]
    ...
    @staticmethod
    def load_obj(filename: str) -> UV3dMesh:
        mesh = UV3dMesh()
        base_dir = os.path.dirname(filename)
        ...
        for item in sub_mesh_data_list:
            ...
            if "map_Kd" in mtl_info:
                ...
                # objファイルを直接いじるんだね
                print(
                    '\033[33mWarning\033[39m : .obj file of 3D model does not have map_Kd Option.\nWhen this option does not exsit , Texture is ".obj default color" or pink .'
                )
            ...
            # 3. メッシュ作成
            s_mesh = UV3dMeshSub(material, mesh_data)
